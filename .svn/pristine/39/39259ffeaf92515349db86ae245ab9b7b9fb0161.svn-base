<template>
  <div class="HotDecCategoryConfig">
    ...
    <Modal v-model="addShow" title="新增">
      <Form :model="addData" ref="addForm" :rules="ruleValidate" :label-width="90">
        ...
        <FormItem label="目的地名称" prop="typeName">
          <Input v-model="addData.subjectName" placeholder="请输入目的地分类名称"></Input>
        </FormItem>
        <FormItem label="目的地描述" prop="typeDetail">
          <Input type="text" v-model="addData.subjectTitle" placeholder="请输入目的地分类描述"></Input>
        </FormItem>
    ...
    <Modal v-model="editShow" title="编辑">
      <Form :model="editData" ref="editForm" :rules="ruleValidate" :label-width="90">
        ...
        <FormItem label="目的地描述" prop="typeDetail">
          <Input type="text" v-model="editData.subjectTitle" placeholder="请输入类型描述"></Input>
        </FormItem>
        <FormItem label="排序" prop="sort">
    ...
    <!--显示列表-->
    <Modal v-model="categoryList" title="产品列表" width="1000">
      <Table :data="productList" :columns="ProductListTab" :loading="loading" border></Table>
      <Page width="100%" @on-change="paging" :total="totalPage" show-elevator style="text-align:right;margin-top: 20px;"></Page>
    </Modal>
    <!--删除产品-->
    <Modal v-model="delProductShow" width="360">
      <p slot="header" style="color:#f60;text-align:center">
        <Icon type="information-circled"></Icon>
        <span>删除确认</span>
      </p>
      <div style="text-align:center">
        <p>确认要移除 < {{ delProductFlag.productName }} > 与专题的关联吗?</p>
      </div>
      <div slot="footer">
        <Button type="error" size="large" long :loading="delLoading" @click="delProduct">确认</Button>
      </div>
    </Modal>
  </div>
</template>
  ...
  export default {
            ...
            render: (h, params) => {
              return h('div', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.deleteProductShow(params.row);
                    }
                  }
                }),
              ])
    ...
    methods: {
      ...
      getList() {
        ...
        this.axios.post(`${this.AjaxUrl}/ty_manage/subject/getList`, this.qs.stringify(data)).then(res => {
      ...
      add(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            this.axios.post(`${this.AjaxUrl}/ty_manage/subject/insert`,
      ...
      editModelShow(e) {
        // console.log(e)
        this.editData.subjectId = e.subjectId;
       ...
       */
      edit() {
        this.axios.post(`${this.AjaxUrl}/ty_manage/subject/update`,
      ...
      * */
      listModelShow(row){
        ...
        this.delProductFlag.subjectId=row.subjectId;
        this.queryProductList.subjectId=row.subjectId;
        this.getProductList();
      },
      deleteProductShow(row){
        this.delProductShow=true;
        this.delProductFlag.productName=row.productName;
        this.delProductFlag.productId=row.productId;
      },
      delProduct(){
        this.delLoading = true;
        var subjectId=this.delProductFlag.subjectId;
        this.axios.post(`${this.AjaxUrl}/ty_manage/product/subject/remove`,
          this.qs.stringify(this.delProductFlag)
        ).then(res => {
          if (res.errorCode === 200) {
            this.$Message.info("删除成功");
            this.getList();
            this.queryProductList.subjectId=subjectId;
            this.getProductList();
      ...
      getProductList(){
        this.axios.post(`${this.AjaxUrl}/ty_manage/subject/queryProductBySubject`,
          this.qs.stringify(this.queryProductList)
        ).then(res => {
          if (res.errorCode === 200) {
            this.productList=res.data.list;
            this.totalPage = res.data.total;
            this.$Message.info("检索成功");
          }
        }).catch(err => {
       ...
       */
      paging(pageNum) {
        this.queryProductList.pageNum = pageNum;
        this.getProductList();
      },
    },
    mounted() {
