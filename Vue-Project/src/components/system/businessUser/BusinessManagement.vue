  ...
  <div class="BusinessManagement">
    ...
    <div v-show="businessInfo">
      <div v-if="!info">
        ...
        <Page width="100%" :total="total" @on-change="chang" :current.sync="condition.pageNum" show-elevator style="text-align:right"></Page>
      </div>
    </div>  
    <div v-if="info">
    ...
    <div v-show="agencyShow">
      <Button type="info" @click="backBusiness">返回</Button><br>
      <h3 style="margin:8px 0;">{{businessName}} > 代理商列表</h3>
      <div>
        <Table border :columns="agencyTop" :loading="agencyLoading" :data="agencyData"></Table>
        <Page width="100%" :total="agencyTotal" @on-change="agencyChang" show-elevator style="text-align:right"></Page>
      </div>
    </div>
    <!--审核通过弹窗-->
    <Modal
      ...
      <div>
        ...
        <strong>推送内容：</strong><br>
        <Input v-model="businessUserInfo.pushText" style="margin:4px 0;width:200px;"></Input>
        <span>现已入驻同游啦！爱旅游的你不来看看吗？</span>
      </div>
    </Modal>
    <!--审核不通过弹窗-->
    <Modal
    ...
    <Modal
      ...
      @on-cancel="cancel">
      <div>
        <strong>商家：{{businessUserInfo.enterpriseName}}</strong><br>
        <strong>序号：</strong>
        <InputNumber v-model="sort" :max="99" :min="1" style="margin:4px 0;width:200px;"></InputNumber>
      </div>
      <div slot="footer">
        <Button type="text" size="large" @click="sortCancel">取消</Button>
        <Button type="primary" size="large" :loading="sortLoading"  @click="sortSub">确认</Button>
      </div>
    </Modal>
  </div>
</template>
<script>
  ...
  export default {
      ...
      return {
              ...
              return h("div", [
                ...
                h(
                  "Button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.addSort(params.row.businessUserInfoId,params.row.sort,params.row.enterpriseName);
                      }
                    }
                  },
                  "修改"
              ...
              return h("div", [
                ...
                h(
                  "Button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.agency(params.row.businessUserId,params.index);
                      }
                    }
                  },
                  "代理关系"
        ...
        agencyTop: [
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if(params.row.companyType == "PERSONAL") {
                text = "个人";
              }else if(params.row.companyType == "COMPANY") {
                text = "企业";
              }
              return h("div", [
                h("strong", text)
          ...
          },
          {
            ...
            render: (h, params) => {
              let time = "";
              if(params.row.applyTime == "" || params.row.applyTime == null){
                time = "";
              }else {
                time = getDate(params.row.applyTime);
              }
              return h("div", [
                h("strong", time)
              ]);
    ...
    watch: {
      "businessUserInfo.enterpriseName": {
      handler: function(val, oldVal) {
        this.businessUserInfo.pushText = this.businessUserInfo.enterpriseName;
        console.log(this.businessUserInfo.pushText);
      },
      deep: true
      },
      applicationtime() {
    ...
    methods: {
      ...
      addSort(id,sort,name) {
        this.modal3 = true;
        this.businessUserInfo.enterpriseName = name;
        this.sort = sort;
        this.businessUserInfoId = id;
      },
      sortSub() {
        this.sortLoading = true;
        let data = {};
        data.businessUserInfoId = this.businessUserInfoId;
        data.sort = this.sort;
        this.axios.post(
            `${this.AjaxUrl}/ty_manage/business/user/info/setSort`,
      ...
      agencyChang(pageNum) {
        ...
        this.agency(this.businessUserI,this.index);
      },
      // 代理》商家
      backBusiness() {
      ...
      // 代理关系
      agency(businessUserId,index) {
        this.businessInfo = false;
        this.agencyShow = true;
        this.agencyLoading = true;
        this.index = index;
        this.businessName = this.Tabledata[this.index].enterpriseName;
        this.businessUserId = businessUserId;
        let data = {};
        data.pageNum = this.pageNums;
        data.businessUserId = this.businessUserId;
        this.axios
          .get(`${this.AjaxUrl}/ty_manage/agent/user/info/getListByBusiness`, {
            params: data
          })
          .then(res => {
            this.agencyData = res.data.list;
      ...
      back() {
        this.info = false;
        console.log(this.condition.pageNum);
        this.Comfort();
      },
      // 搜索
      search() {
